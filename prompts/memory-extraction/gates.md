...
EXPECTED YIELD (calibration only — NOT a target; never pad to fill and never drop to hit a number):
   ...
   Returning [] is correct when nothing durable was learned. A long, substantive session can legitimately exceed the typical ranges above: when the session genuinely contains that much durable gold, KEEP every candidate that independently passes ALL the gates — do not truncate real, gate-passing knowledge just to stay small. HARD MAX 20 (a backstop, not a quota and NOT a target to approach): a memory is emitted ONLY because it independently passes the gates, NEVER to fill toward this number; padding to reach it is a failure. If more than 20 gate-passing memories survive, emit the highest-ranked by data-loss/security/corruption prevention > failure+fix > stable convention > reusable technique > lower-risk process rule.
