...
describe('loadMemberships', () => {
  ...
  it('parses response into OrgMembership array and fetches envelopes from hub', async () => {
    ...
    const envelopeUrl = mockFetch.mock.calls[1][0] as string;
    const envelopeOpts = mockFetch.mock.calls[1][1] as { headers?: Record<string, string> };
  ...
  it('fetches envelopes for each org in the list', async () => {
    ...
    const envelopeUrlA = mockFetch.mock.calls[1][0] as string;
    const envelopeUrlB = mockFetch.mock.calls[2][0] as string;
  ...
  it('unseals mod_envelope for leader role', async () => {
    ...
    vi.mocked(openEnvelope).mockReturnValueOnce(unsealedModKey);
    ...
    expect(memberships[0].modPrivkey).toBeTruthy();
